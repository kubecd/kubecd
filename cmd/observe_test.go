...
func TestImageReleaseIndex(t *testing.T) {
	...
	assert.Len(t, index["test-image"], 2)
	assert.Len(t, index["test-image2"], 1)
	assert.Equal(t, "release1", index["test-image"][0].Name)
	assert.Equal(t, "release2", index["test-image"][1].Name)
	assert.Equal(t, "release3", index["test-image2"][0].Name)
}
